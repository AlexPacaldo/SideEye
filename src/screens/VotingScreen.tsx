...
function VotingPassPlay() {
  ...
  const [selected, setSelected] = useState<string | null>(null)
  const candidates = candidatesFor(room, me.playerId)
  const clues = clueMap(room)
  ...
  return (
    <PassPhoneScreen
      ...
      gateHint="Pass it along. Votes stay secret."
      revealHint="Who's acting sus?"
      onProceed={() => void safe(backend.runPassTurn())}
      onHide={() => void safe(backend.runPassTurn())}
    >
      <div className="col" style={{ gap: 18, width: '100%' }}>
        <VoteTarget
          players={candidates}
          clues={clues}
          selected={selected}
